...
def write_indexed_sumstats(directory,marker_names,trait_names,n_samples,chunks,
 ...
 for chunk in chunks:
  ...
  if kind=='significant':
   ...
   emit({'variant_index':np.asarray(vi,dtype=np.int64),'trait_index':np.asarray(ti,dtype=np.int64),'beta':np.asarray(beta),'t_stat':np.asarray(t)})
   continue
  ...
  if heaps is None:
   emit({'variant_index':(start+vi).astype(np.int64),'trait_index':np.asarray(trait_index,dtype=np.int64),'beta':beta[vi,ti],'t_stat':t[vi,ti]})
  else:
 ...
 if heaps is not None:
  ...
  emit({key:np.asarray(v,dtype=np.int64 if key.endswith('_index') else np.float64) for key,v in values.items()})
 ...
 manifest={'format':'torchgwas-indexed-sumstats','version':1,'kind':'jagwas' if kind=='jagwas' else 'linear','shape':[len(marker_names),len(trait_names)],'n_samples':int(n_samples),'df':int(chi2_df if kind=='jagwas' else df),'traits':list(trait_names),'parts':parts,'rows':total,'variant_ids':'variant_ids.npy','p_value':'derived from chi2 and df' if kind=='jagwas' else 'two-sided Student t on t_stat with df'}
